---
title: "metabr Developer Guide Overview & Architecture"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{metabr Developer Guide Overview & Architecture}
...
**metabr** (v0.2.3) is an R package within the `foundr` / systems genetics ecosystem providing specialized tools for metabolomics data ingestion, mass spectrometry peak-area quality control (QC) batch correction, normalization, and curation discrepancy analysis.
...
1. **[Developer Guide Overview & Architecture](index.html)**: Package purpose, ecosystem architecture, end-to-end processing flowchart, and quick start instructions.
2. **[QC Pipeline & Correction Methodology](qc_pipeline.html)**: Mathematical specifications for QC correction factors, compound-wise median retention time (`medRt`) handling, missing value imputation, and `hand_auto` curation diagnostics.
3. **[Data Ingestion & IO Format Specifications](data_ingestion.html)**: Detailed schemas for raw mass spectrometry spreadsheets, minute time-course files, and long-format tidy data frame representations.
